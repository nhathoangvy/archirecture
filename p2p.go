...
var (
	...
	localHTTPPort, remoteHTTPPort, localMuxPort, remoteMuxPort int
)
...
func main() { //nolint
	...
	flag.BoolVar(&isControlling, "middle", false, "is ICE Agent middle")
	flag.Parse()
	...
	addr := &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1), Port: localMuxPort}
	mUdpServer, err := net.ListenUDP("udp4", addr)
	if err != nil {
		log.Print(err)
	}
